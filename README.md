...
Renderer сохраняет глобальный header с выбором контекста, namespace, языка и
темы. Рабочая поверхность под ним намеренно пуста и служит основанием для
нового интерфейса Studio.
...
preload-мост. Закрытие скрывает окно в системный tray; клик по иконке возвращает
и фокусирует Studio, а выход выполняется через меню tray. Кнопки сворачивания и
закрытия обе скрывают окно в tray. На Linux аппаратный video decode/encode
отключён, чтобы Studio без видеоповерхностей не инициализировала VA-API; GPU-
композитинг интерфейса остаётся включённым.
